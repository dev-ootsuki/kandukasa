<template>
    ...
    <q-table
        ...
        virtual-scroll
        :visible-columns="headerVisibleIndexes"
        selection="multiple"
        v-model:selected="multiSelected"
    >
        <template v-slot:top-left>
            <SystemBtnOperation mode="register" feature="dbdata" @click="onCreateIndex" />
            <q-space class="q-pl-md" />
            <SystemBtnOperation mode="bulk_delete" feature="dbdata" @click="onBulkDeleteIndexes" />
        </template>

        <template v-slot:top-right>
            <q-space class="q-pl-md" />
            <q-select
                v-model="headerVisibleIndexes"
                multiple
                outlined
                dense
                options-dense
        ...
        <template v-slot:body-cell="props">
            <q-td :props="props">
                <span v-if="props.col.name == 'id'">
                    <SystemBtnOperation mode="delete" feature="dbfeatures" mini @click="onDeleteIndex(props.row)" />
                </span>
...
import { DbIndex, DbColumn } from '~/types/Domain.class'
...
// FKEY定義
// view上のヘッダ定義
const headerIndexes = TableHelper.createIndexColumns(t)
// ヘッダを画面上で表示/非表示切り替えする時の見えるリスト
const headerVisibleIndexes = ref(headerIndexes.map(e => e.name))
// 操作するのでindexesをpropsからrefにしておく
const indexes = ref(props.indexes == null ? [] : props.indexes)
// 一括選択用
const multiSelected = ref<DbIndex[]>([])
// 単一項目削除用
const selectedRow = ref<DbIndex>()
// 登録用
const registrationDialog = useTemplateRef<any>("registrationDialog")

const onDeleteIndex = (row:any) => {
  selectedRow.value = row
  dialog.value.show("delete")
}
const onBulkDeleteIndexes = () => {
...
const handler:Design.MultiDialogHandler = {
  delete: {
    submit:() : Promise<any> => {
      return store.deleteIndexes([selectedRow.value!])
    },
    complete: () => {
      store
  ...
  bulk_delete: {
    submit: () : Promise<any> => {
        return store.deleteIndexes(multiSelected.value)
    },
    complete: () => {
        store
        .getTableInfo(store.selectedTable!.table_id!)
